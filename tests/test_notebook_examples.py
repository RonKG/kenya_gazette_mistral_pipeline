...
PROTOTYPE_NOTEBOOK = ROOT / "gazette_etl_prototype.ipynb"
...
OPTIONAL_ROOT_IMPORTS = {"get_envelope_schema", "validate_envelope_json"}
...
def test_recommended_notebook_bootstraps_source_checkout_imports() -> None:
    source = _cell_source(_load_notebook(DRIVER_NOTEBOOK)["cells"][1])
...
def test_recommended_notebook_default_is_offline_replay() -> None:
    source = _notebook_source(DRIVER_NOTEBOOK)
    active_code = _non_comment_code_source(DRIVER_NOTEBOOK)

    assert "replay_raw_json_path" in source
    assert "tiny_replay.raw.json" in source
    assert "parse_url(PDF_URL, config=config)" in source
    assert "allow_live_mistral" not in active_code
...
def test_docs_point_to_thin_driver_and_offline_policy() -> None:
    ...
    assert "gazette_etl_prototype.ipynb" in readme
    prototype_mentions = [line for line in readme.splitlines() if "gazette_etl_prototype.ipynb" in line]
