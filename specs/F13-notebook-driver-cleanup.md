...
| Input examples | Use a PDF URL string for the primary reproducible example. After F14, a local PDF example may also show `parse_file(...)` live upload behavior, but should stay optional because it depends on a user-specific local path |
